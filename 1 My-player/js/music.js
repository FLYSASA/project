...
var Fm = {
    init : function(){
        ...
        this.$voiceBar = this.$container.find('.aside .voice-bar')
        this.$voiceProgressBar = this.$container.find('.aside .voice-progress')
        this.audio = new Audio()
        this.audio.autoplay = true
        //初始化音量
        this.audio.volume = 0.5
        this.$voiceProgressBar.css('width',this.audio.volume*100+'%')
    ...
    bind: function(){           //第一件绑定的事是对监听到的fire数据设定行为函数
        ...
        _this.$container.find('.aside .bar').on('click',function(event) {
            ...
            var second = Math.floor(_this.audio.currentTime%60)+''                 //取余数,注意当second的是个位数时候记得补个0,这里记得+''转成字符串
            second = second.length === 2?second:'0'+second
            _this.$container.find('.aside .current-time').text(min+':'+second)    //html 时间更新

            _this.$container.find('.aside .bar-progress').css('width',_this.audio.currentTime/_this.audio.duration*100+'%')
        })

        //音量控制
        _this.$container.find('.aside .voice-bar').on('click',function(event){
            var voiceBarWidth = _this.$voiceBar[0].clientWidth   //注意clientWidth是dom对象的属性,需要将jQuery对象转为dom对象,不转的话可以用outWidth属性
            var newVolume = event.offsetX / voiceBarWidth
            _this.audio.volume = newVolume
            _this.$voiceProgressBar.css('width',_this.audio.volume*100+'%')
        })

        //静音
        _this.$container.find('.no-voice').on('click',function() {
            if(!_this.audio.muted){
                _this.audio.muted = true
                _this.$voiceProgressBar.css('width',0+'%')
            }else{
    ...
    updateStatus: function(){
        ...
        var totalMin = Math.floor(this.audio.duration/60)
        var totalSecond = Math.floor(this.audio.duration%60)+''
        totalSecond = totalSecond.length === 2?totalSecond:'0'+totalSecond

        this.$container.find('.aside .current-time').text(min+':'+second)    //html 时间更新
        this.$container.find('.aside .bar-progress').css('width',_this.audio.currentTime/_this.audio.duration*100+'%')
        this.$container.find('.aside figure').css('animation','xuanzhuan 5s linear infinite')
    ...
    setMusic: function(){
        ...
        this.$container.find('.aside figure').css('animation','xuanzhuan 5s linear infinite')  /*linear是线性变化 infinite是永久循环 */
        this.$container.find('.aside h3').text(this.song.title)
